...
SearchProductByCode (void)
{
	...
	{
		mayorista = strcmp (PQgetvalue (res, 0, 9), "t") == 0 ? TRUE : FALSE;

		gtk_entry_set_text (GTK_ENTRY (venta->product_entry),
							g_strdup_printf ("%s  %s  %s  %s", PQgetvalue (res, 0, 1),
											 PQgetvalue (res, 0, 2), PQgetvalue (res, 0, 3),
											 PQgetvalue (res, 0, 4)));

		stock = strtod (PUT (PQgetvalue (res, 0, 5)), (char **)NULL);

		stockday = GetDayToSell (PQgetvalue (res, 0, 10));

		if (stock <= GetMinStock (PQgetvalue (res, 0, 10)))
		...
		gtk_entry_set_text (GTK_ENTRY (venta->codigo_entry), PQgetvalue (res, 0, 0));
		gtk_entry_set_text (GTK_ENTRY (venta->barcode_entry), PQgetvalue (res, 0, 10));
		if (precio != 0)
		{
			if( venta_directa == 1 ) {
				if( VentaFraccion( PQgetvalue( res, 0, 8 ) ) ) {
