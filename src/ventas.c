...
CanjearProductoWin (GtkWidget *widget, gpointer data)
{
  ...
  label = gtk_label_new ("C贸digo de Barras: ");
...
ventas_win ()
{
  ...
  column = gtk_tree_view_column_new_with_attributes ("C贸digo", renderer,
                                                     "text", 0,
                                                     NULL);
  ...
  renderer = gtk_cell_renderer_text_new ();
  column = gtk_tree_view_column_new_with_attributes ("Descripci贸n", renderer,
                                                     "text", 1,
                                                     NULL);
  gtk_tree_view_append_column (GTK_TREE_VIEW (venta->treeview_products), column);
  gtk_tree_view_column_set_alignment (column, 0.5);
      ...
      if (strcmp (codigo, "") != 0)
        {
          AlertMSG (GTK_WIDGET (gtk_builder_get_object (builder, "barcode_entry")), g_strdup_printf
                    ("No existe un producto con el c贸digo %s!!", codigo));
      ...
      if (VentaFraccion (barcode) == FALSE)
        {
          aux_widget = GTK_WIDGET (gtk_builder_get_object (builder, "cantidad_entry"));
          AlertMSG (aux_widget,
                    "Este producto no se puede vender por fracci贸n de producto");
...
/**
 * Al recibir un signal de "cantidad_entry", cambia el foco dependiendo de la opci髇
 * VENTA_DIRECTA especificada en el .rizoma
 *
 * @parametro entry tipo GtkEntry * que recive la se馻l
...
{ // Al presionar [ENTER] en el entry "Cantidad"
  if(!atoi(rizoma_get_value("VENTA_DIRECTA"))) // Si VENTA_DIRECTA = 0 mueve el foco al bot髇 "A馻dir"
    {
      GtkWidget *button;
  ...
  else // De lo contrario el foco regresa al entry "C骴igo de barras"
      gtk_widget_grab_focus( GTK_WIDGET (gtk_builder_get_object (builder, "barcode_entry")));
}
...
TipoVenta (GtkWidget *widget, gpointer data)
{
  ...
  /*Verifica si hay productos a馻didos en la TreeView para vender*/
      ...
      if (strcmp (barcode, "") != 0)
        {
          AlertMSG (widget, g_strdup_printf
                    ("No existe un producto con el c贸digo de barras %s!!", barcode));
        ...
        if (GetCurrentStock (barcode) == 0)
          {
            AlertMSG (widget, "No ahi mercader铆a en Stock.\nDebe ingresar mercader铆a");
  ...
  if (gtk_tree_view_get_model (treeview) == NULL )
    {
      ...
      if (solo_venta != TRUE)
        {
          renderer = gtk_cell_renderer_text_new ();
          column = gtk_tree_view_column_new_with_attributes ("C贸digo de Barras",
                                                             renderer,
                                                             "text", 1,
                                                             NULL);
          gtk_tree_view_append_column (treeview, column);
      ...
      renderer = gtk_cell_renderer_text_new ();
      column = gtk_tree_view_column_new_with_attributes ("Descripci贸n",
                                                         renderer,
                                                         "text", 2,
                                                         "foreground", 8,
                                                         "foreground-set", 9,
...
AskCancelSell (GtkWidget *widget, gpointer data)
{
  ...
  label = gtk_label_new ("驴Desea cancelar la venta?");
  gtk_box_pack_start (GTK_BOX (hbox), label, FALSE, FALSE, 3);
  gtk_widget_show (label);
  ...
  if (sale_id != NULL)
    {
      if (atoi(sale_id) == 0)
        {
          AlertMSG(GTK_WIDGET(entry), "Debe ingresar un N潞 de Venta mayor a 0\nSi no sabe el numero de venta deje el campo vacio");
          return;
        }
      else
...
realizar_traspaso_Env (GtkWidget *widget, gpointer data)
{
  ...
  /*Comprueba que hallan productos a馻didos en el treView para traspasar */
