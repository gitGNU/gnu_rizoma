...
compras_win (void)
{
  ...
  gtk_entry_set_max_length (GTK_ENTRY (builder_get (builder, "entry_buy_amount")), 8);
...
on_entry_srch_provider_activate (GtkEntry *entry)
{
  ...
  if (tuples > 0)
    {
      gtk_widget_grab_focus (GTK_WIDGET (builder_get (builder, "tree_view_srch_provider")));
      gtk_tree_selection_select_path (gtk_tree_view_get_selection (GTK_TREE_VIEW (builder_get (builder, "tree_view_srch_provider"))),
                                      gtk_tree_path_new_from_string ("0"));
    }
  else
